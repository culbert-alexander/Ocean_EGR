...
for ifreq = 1 : freq_index
   ...
   if ( ifreq < freq_index )
      iRecProfile = iRecProfile + 2 + Modes.M + 1 + floor( ( 2 * Modes.M - 1 ) / lrecl );   % advance to next profile
...
iRecProfile = iRecProfile + 3 + Modes.M + floor( ( 2 * Modes.M - 1 ) / lrecl );   % advance to next profile
